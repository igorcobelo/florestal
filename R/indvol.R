indvol <- function(x, mens="plot", vol=FALSE, myeq=NULL, veg=NULL, f=NULL, circ=FALSE) {
  ...
  df <- x
    ...
    IF_eq <- df %>%
      group_by(ind) %>%
      mutate(dap_equivalente = sqrt(sum(d^2, na.rm=T))) %>%
      summarise(Plot=unique(parcela),
                Individual=unique(ind),
                Specie=unique(spp),
                `Height (m)`= unique(h),
                dap_equivalente = unique(dap_equivalente))

    d<- as.numeric(IF_eq$dap_equivalente)
  ...
  if(mens=="census"){
    ...
    IF_eq <- df %>%
      group_by(ind) %>%
      mutate(dap_equivalente = sqrt(sum(d^2, na.rm=T))) %>%
      summarise(Individual=unique(ind),
                Specie=unique(spp),
                `Height (m)`= unique(h),
                dap_equivalente = unique(dap_equivalente))
